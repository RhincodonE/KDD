...
def clustering(net,h_structure, X_train):
    ...
    X_scaled = scaler.fit_transform(all_images_np)
...
# Loop over the number of clusters at each level and perform KMeans clustering

    for i in range(len(n_clusters)):
        
        print(i)
        
        kmeans = KMeans(n_clusters=n_clusters[i], random_state=42)
        ...
        if i == 0:
            ...
            kmeans.fit(clusters[i-1].reshape(-1, 1).astype(np.float32))
            
        clusters.append(kmeans.predict(X.astype(np.float32)))
    ...
    for j in range(X.shape[0]):
        ...
        for i in range(len(n_clusters)):
            ...
            sample_cluster = clusters[i][j]
            
            sample_label.append(sample_cluster)
            
        sample_labels.append(sample_label)

    print(sample_labels[0])

    print(sample_labels[0:10])

    print(np.array(sample_labels).shape)
