...
Section('data', 'data related stuff').params(
    finetune_dataset=Param(str, 'file to store in datasets', required=True),
...
@param('data.finetune_dataset')
@param('data.val_dataset')
@param('training.batch_size')
@param('training.num_workers')
@param('data.gpu')
def make_dataloaders(finetune_dataset=None, val_dataset=None, batch_size=None, num_workers=None, gpu=0):
    paths = {
    ...
    for name in ['finetune', 'test']:
        ...
        ordering = OrderOption.RANDOM if name == 'finetune' else OrderOption.SEQUENTIAL
...
def fine_tune(model, loaders, lr=None, epochs=None, label_smoothing=None,
          momentum=None, weight_decay=None, lr_peak_epoch=None):
    opt = SGD(model.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
    iters_per_epoch = len(loaders['finetune'])
    ...
    for _ in range(epochs):
        for ims, labs in tqdm(loaders['finetune']):
...
def load_model(device):
    ...
    model.load_state_dict(ch.load("/users/home/ygu27/cifar/models_f/model_in.pth", map_location=device))
    return model
...
if __name__ == "__main__":
    ...
    model_in = load_model(f'cuda:{config["data.gpu"]}')
    fine_tune(model_in, loaders)
