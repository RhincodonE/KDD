...
def create_custom_dataloader(loader, device):
    ...
    for img_batch, label_batch in tqdm(loader):
        # Ensure all operations are moved off the GPU to save memory
        img_batch = img_batch.cpu().detach()
        label_batch = label_batch.cpu().detach()
        labels.append(label_batch)  # Handle a single integer label

        # Handle images, should always be a tensor
        images.extend(img_batch.tolist())
    images = ch.tensor(images)
    print(images.shape)
    labels = ch.tensor(labels)
    print(labels.shape)
    dataset = CustomDataset(images, labels)
    new_loader = DataLoader(dataset, batch_size=512, shuffle=False, num_workers=loader.num_workers)
    loader_1 = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=loader.num_workers)
    return new_loader,loader_1
...
@param('data.gpu')
@param('data.unlearning')
@param('data.unlearning_index')
def cal_p(model, in_dataset_index, out_dataset_index, loaders, gpu, unlearning, unlearning_index):
    logits = []
    index = 0
    in_index = np.load(in_dataset_index)
    out_index = np.load(out_dataset_index)
    if unlearning:
        indices_data = pd.read_csv(unlearning_index)
        top_indices = indices_data['Index'].tolist()

        original_indexes = list(range(50000))  # Original list of indexes from 0 to 49999

        projection_dict = {}  # Dictionary to store the projection between current and original indexes
  # Offset to adjust for the removal of indexes
        current_index = 0  # Current index after removal

        for original_index in original_indexes:
            if original_index not in top_indices:
                projection_dict[current_index] = original_index
                current_index += 1

        if len(projection_dict.items()) == (len(original_indexes)-len(top_indices)):
            print('pass:length check')

        for i in projection_dict.items():
            if i in top_indices:
                print(f"Failed: index{i} is not removed")
        print('pass:logit check')
    ...
    if model is not None:
        model.eval()
        for name in ['train_out','train_in']:
            with ch.no_grad():
                for ims, labs in tqdm(loaders[name]):
                    out = model(ims.to(f'cuda:{gpu}').float())
                    ...
                    logits.append({"index": indices[name][index] if not unlearning else projection_dict[indices[name][index]] , "True_label": name, "Prediction": prediction})
                    index = index+1
            index = 0
...
def load_model(model_path, device, loaders):
    model = construct_model()  # Assuming this function correctly constructs the model

    privacy_engine = PrivacyEngine()
    opt = SGD(model.parameters(), lr=0.5, momentum=0.9, weight_decay=5e-4)
...
if __name__ == "__main__":
    ...
    for model_filename in tqdm(os.listdir(config['data.model_folder']), desc=f'Attacking'):
        ...
        if model_filename.endswith(('.pt', '.pth')):
            ...
            out_index_file_path = os.path.join(config['data.dataset_folder'], f'out_DP_{model_number}.npy')
            in_index_file_path = os.path.join(config['data.dataset_folder'], f'in_DP_{model_number}.npy')

            in_set = os.path.join(config['data.dataset_folder'], f'cifar_in_DP_{model_number}.beton')
            out_set = os.path.join(config['data.dataset_folder'], f'cifar_out_DP_{model_number}.beton')
            test_set = os.path.join(config['data.dataset_folder'], 'cifar_test.beton')


            logging.info(f"Processing model number: {model_number}")
            results_file_path = os.path.join(config['data.attacking_results_folder'], f'result_DP_{model_number}.csv')
            loaders = make_dataloaders(in_dataset=in_set, out_dataset=out_set, val_dataset=test_set)
            gpu = config['data.gpu']
            device = f'cuda:{gpu}'
            custom_loader,loader_to_use = create_custom_dataloader(loaders['train_in'], device)
            loaders['train_in'] = custom_loader

            print('loading model')
            model = load_model(model_path, device, loaders)
            print('cal_p')
            loaders['train_in'] = loader_to_use
